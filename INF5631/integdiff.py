...
def trapezoidal_PETSc(f, a, b, n):
	...
	be used in parallel.
    """
	# Let x be distributed too!
	# This is fine as a 1D integral example, but should use
	# DA for multiple dimensions.
	x = np.linspace(a, b, n+1)
	h = (b-a)/float(n)
	t0 = time.clock()
	# Keep f as function, f(x[Istart:Iend])
	f_vec = f#(x)
	f_vec[0] /= 2.0
	f_vec[-1] /= 2.0
	...
	# Now we need to make a local numpy array of length Istart-Iend+1
	# and fill it, then insert with Pf.setValues
	Pf.setValues(range(Istart,Iend),f_vec[Istart:Iend])
	# Modify /= 2 if Istart == 0 or Iend == f_vec.size()-1
...
def differentiate_PETSc(f, a, b, n):
	...
	"""
	# Note: should use a DA in 1D and 2D for holding f values,
	# call it u and have f(x,y)
	# Each processor performs its operation in calculating
	# the gradient, using ghost points from neighbors. Most
	# of the explicit FD schemes will with this example
